...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, FGS} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, FGS} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, FGO} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, FGO} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, FGK} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, FGK} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
