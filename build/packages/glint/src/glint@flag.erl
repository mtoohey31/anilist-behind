...
-opaque flag_builder(FXJ) :: {flag_builder,
        binary(),
        fun((binary()) -> {ok, FXJ} | {error, snag:snag()}),
        fun((internal(FXJ)) -> value()),
        gleam@option:option(FXJ)}.

-opaque internal(FXK) :: {internal,
        gleam@option:option(FXK),
        fun((binary()) -> {ok, FXK} | {error, snag:snag()})}.

-type flag() :: {flag, value(), binary()}.

-spec new(
    fun((internal(FYB)) -> value()),
    fun((binary()) -> {ok, FYB} | {error, snag:snag()})
) -> flag_builder(FYB).
new(Valuer, P) ->
...
-spec attempt(
    {ok, FYS} | {error, FYT},
    fun((FYS) -> {ok, any()} | {error, FYT})
) -> {ok, FYS} | {error, FYT}.
attempt(Val, F) ->
    gleam@result:'try'(Val, fun(A) -> gleam@result:replace(F(A), A) end).

-spec wrap_with_constraint(
    fun((binary()) -> {ok, FYM} | {error, snag:snag()}),
    fun((FYM) -> {ok, nil} | {error, snag:snag()})
) -> fun((binary()) -> {ok, FYM} | {error, snag:snag()}).
...
-spec constraint(
    flag_builder(FYI),
    fun((FYI) -> {ok, nil} | {error, snag:snag()})
) -> flag_builder(FYI).
...
-spec description(flag_builder(FZB), binary()) -> flag_builder(FZB).
description(Builder, Description) ->
    erlang:setelement(2, Builder, Description).

-spec default(flag_builder(FZE), FZE) -> flag_builder(FZE).
...
-spec construct_value(binary(), internal(FZO), fun((internal(FZO)) -> value())) -> {ok,
        value()} |
    {error, snag:snag()}.
...
-spec get_value(
    gleam@dict:dict(binary(), flag()),
    binary(),
    fun((flag()) -> {ok, FZU} | {error, snag:snag()})
) -> {ok, FZU} | {error, snag:snag()}.
