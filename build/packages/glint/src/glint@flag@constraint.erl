...
-spec one_of(list(FVS)) -> fun((FVS) -> {ok, nil} | {error, snag:snag()}).
one_of(Allowed) ->
...
-spec none_of(list(FVV)) -> fun((FVV) -> {ok, nil} | {error, snag:snag()}).
none_of(Disallowed) ->
...
-spec each(fun((FVY) -> {ok, nil} | {error, snag:snag()})) -> fun((list(FVY)) -> {ok,
        nil} |
    {error, snag:snag()}).
each(Constraint) ->
    fun(L) -> _pipe = L,
